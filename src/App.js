...
  };
  
  const primaryBtnClass = `bg-${theme.primary}-${theme.isDarkMode ? '600' : '500'} hover:bg-${theme.primary}-${theme.isDarkMode ? '700' : '600'} text-white shadow-lg`;
  const primaryTextClass = `text-${theme.primary}-${theme.isDarkMode ? '400' : '600'}`;
  const secondaryBtnClass = theme.isDarkMode ? `bg-gray-700 hover:bg-gray-600 text-white` : `bg-gray-300 hover:bg-gray-400 text-gray-800`;
  const secondaryTextClass = theme.isDarkMode ? `text-gray-400` : `text-gray-600`;
...
        <div className={`p-8 rounded-xl shadow-lg max-w-sm w-full ${cardBgClass}`}>
          <div className="flex flex-col items-center mb-6">
            <Lock size={48} className={`mb-4 ${primaryTextClass}`} />
            <h2 className={`text-3xl font-bold ${primaryTextClass}`}>Admin Login</h2>
          </div>
...
              Unlock Admin Panel
            </button>
            <button type="button" onClick={() => setShowLogin(false)} className={`w-full flex items-center justify-center font-bold py-3 px-6 rounded-lg transition duration-200 ${secondaryBtnClass}`}>
              Cancel
            </button>
          </form>
        </div>
...
  return (
    <div className={`flex flex-col md:flex-row min-h-screen font-inter ${bgColorClass} transition-colors duration-500`}>
      <style jsx>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
        body { font-family: 'Inter', sans-serif; }
      `}</style>
...
          <h1 className={`text-4xl lg:text-5xl font-extrabold ${primaryTextClass}`}>{settings.businessName}</h1>
          <button onClick={handleEmployeeSignOut} className={`flex items-center gap-2 font-bold py-3 px-6 rounded-lg transition duration-200 ${secondaryBtnClass}`}>
            <LogOut size={20} />
            Sign Out
          </button>
        </div>
        {Object.keys(categorizedMenuItems).length === 0 ? (
          <p className="text-center text-lg text-gray-500 py-10">No menu items available. Please add some from the Admin Panel.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.keys(categorizedMenuItems).map(category => (
              <div key={category} className="mb-8">
                <h2 className={`text-3xl font-bold mb-4 ${primaryTextClass}`}>{category}</h2>
                <div className="grid gap-4">
                  {categorizedMenuItems[category].map(item => (
                    <button key={item.id} onClick={() => addItemToOrder(item)} className={`relative flex flex-col items-center p-4 rounded-xl shadow-md ${cardBgClass} ${cardHoverClass} transition-all duration-300 transform hover:scale-105`}>
                      <h3 className="text-lg font-semibold">{item.name}</h3>
                      <span className="text-2xl font-bold text-green-400 mt-2">${item.price.toFixed(2)}</span>
                      <PlusCircle size={24} className={`absolute bottom-2 right-2 text-${theme.primary}-500 opacity-0 group-hover:opacity-100 transition-opacity`} />
                    </button>
                  ))}
                </div>
              </div>
...
      {/* Order Sidebar */}
      <div className={`w-full md:w-1/3 lg:w-1/4 ${cardBgClass} shadow-2xl p-6 lg:p-8 flex flex-col ${borderClass} md:border-l`}>
        <h2 className={`text-3xl font-bold mb-6 ${primaryTextClass}`}>Current Order</h2>
        {order.length === 0 ? (
          <p className={`${secondaryTextClass} text-center py-10`}>Order is empty.</p>
        ) : (
          <ul className="flex-1 overflow-y-auto space-y-4 mb-6">
            {order.map(item => (
              <li key={item.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-gray-700">
                <div>
                  <p className="font-semibold">{item.name}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">${item.price.toFixed(2)} ea.</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => updateItemQuantity(item.id, -1)} className={`p-1 rounded-full ${secondaryBtnClass}`}>
                    <MinusCircle size={20} />
                  </button>
...
          <button onClick={sendOrderToKitchen} disabled={order.length === 0} className={`w-full flex items-center justify-center font-bold py-3 px-6 rounded-lg transition duration-200 ${order.length === 0 ? 'bg-gray-400 cursor-not-allowed' : primaryBtnClass}`}>
            <Send className="mr-2" />
            Send to Kitchen
          </button>
          <button onClick={clearOrder} disabled={order.length === 0} className={`w-full flex items-center justify-center font-bold py-3 px-6 rounded-lg transition duration-200 ${order.length === 0 ? 'bg-gray-400 cursor-not-allowed' : secondaryBtnClass}`}>
